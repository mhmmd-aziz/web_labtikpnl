   ...
   public static function getEloquentQuery(): Builder
{
    $user = auth()->user();
    if ($user->role === 'admin') {
        return parent::getEloquentQuery(); // Admin lihat semua
    }
    if ($user->role === 'mahasiswa') {
        // Mahasiswa lihat berdasarkan class_id
        return parent::getEloquentQuery()->where('class_id', $user->class_id);
    }
    if ($user->role === 'dosen') {
         // Dosen lihat berdasarkan dosen_id
        return parent::getEloquentQuery()->where('dosen_id', $user->id);
    }
    ...
    public static function form(Form $form): Form
{
    return $form
        ->schema([
            ...
            Forms\Components\TimePicker::make('jam_mulai')
                ...
                ->required()
                ->seconds(false)
                ->live() // <-- Penting untuk validasi real-time
            ...
            Forms\Components\TimePicker::make('jam_selesai')
                ...
                ->live() // <-- Penting untuk validasi real-time
                ->visible(fn (Get $get) => $get('tipe_jadwal') === 'insidental')
                // [FIX] Memperbaiki struktur closure untuk mencegah timeout
                ->rules([
                    function (Get $get) {
                        return function (string $attribute, $value, \Closure $fail) use ($get) {
                            // Cek awal untuk memastikan semua data yang dibutuhkan ada
                            if ($get('tipe_jadwal') !== 'insidental' || !$get('room_id') || !$get('hari') || !$get('jam_mulai')) {
                                return;
                            }

                            $record = $get('record'); // Cara aman mendapatkan record saat edit
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                // [FIX] Merapikan urutan kolom dan menghapus duplikat
